...
// Duplicate route removed (consolidated at the top)

// Mark all user requests as read
app.post('/api/admin/user-requests/mark-all-read', async (req, res) => {
    try {
        await pool.query('UPDATE user_requests SET status = "read" WHERE status = "new"');
...
// Get all user requests (Used by Admin)
app.get('/api/admin/user-requests', async (req, res) => {
    try {
...
// Send matched tenders email to user
app.post('/api/admin/send-tender-email', async (req, res) => {
    try {
        const { requestId, tenderIds } = req.body;

        if (!requestId || !tenderIds || !Array.isArray(tenderIds)) {
            return res.status(400).json({ message: 'Request ID and Tender IDs array are required' });
        }

        // 1. Fetch user request and user details
        const [requests] = await pool.query(`
        ...
        // 2. Fetch tender details - querying all three tables
        let tenders = [];
        for (const id of tenderIds) {
            // Check each table for the tender
            const [gem] = await pool.query('SELECT *, "GEM" as source FROM gem_tenders WHERE tender_id = ?', [id]);
            const [eproc] = await pool.query('SELECT *, "eProcurement" as source FROM eprocurement_tenders WHERE tender_id = ?', [id]);
            const [ireps] = await pool.query('SELECT *, "iREPS" as source FROM ireps_tenders WHERE tender_id = ?', [id]);

            if (gem.length > 0) tenders.push(gem[0]);
            else if (eproc.length > 0) tenders.push(eproc[0]);
            else if (ireps.length > 0) tenders.push(ireps[0]);
        }

        if (tenders.length === 0) {
            return res.status(404).json({ message: 'No tenders found for provided IDs' });
        }

        // Helper to extract clean titles and valid URLs
        const getCleanTenderData = (t) => {
            let tenderId = t.tender_id || t.id || 'N/A';
            let title = t.name_of_work || t.title || 'N/A';
            let department = t.tender_dept || t.authority || 'Not Specified';
            let state = t.state_name || t.state || 'Not Specified';
            let emd = t.tender_emd || t.emd || 'N/A';
            let ecv = t.tender_ecv || t.estimated_value || 'N/A';
            let deadline = t.closing_date || t.deadline || 'N/A';

            let docLink = t.gemdoclink || t.doclinks || t.tender_link || t.document_link || t.source_site || '';

            // Handle bad scrapes where URL is in title
            if (typeof title === 'string' && (title.startsWith('http://') || title.startsWith('https://'))) {
                docLink = title;
                title = 'Document link provided (Title missing)';
            }

            // Verify the docLink is an actual document, not a generic portal homepage
            let finalDocLink = null;
            if (docLink && docLink.trim() !== '' && docLink !== 'N/A') {
                const cLink = docLink.trim().toLowerCase();
                const genericUrls = [
                    'https://bidplus.gem.gov.in/', 'https://bidplus.gem.gov.in',
                    'https://gem.gov.in/', 'https://gem.gov.in',
                    'https://eprocure.gov.in/', 'https://ireps.gov.in/'
                ];
                if (!genericUrls.includes(cLink)) {
                    finalDocLink = docLink.trim();
                }
            }

            return { tenderId, title, department, state, emd, ecv, deadline, finalDocLink };
        };

        // 3. Always generate Excel attachment (regardless of tender count)
        const exceljs = require('exceljs');
        const workbook = new exceljs.Workbook();
        const worksheet = workbook.addWorksheet('Matched Tenders');
        worksheet.columns = [
            { header: 'S.No', key: 'sno', width: 5 },
            { header: 'Tender ID', key: 'tender_id', width: 22 },
            { header: 'Title/Work Details', key: 'title', width: 60 },
            { header: 'Department', key: 'department', width: 30 },
            { header: 'State', key: 'state', width: 15 },
            { header: 'EMD', key: 'emd', width: 15 },
            { header: 'ECV', key: 'ecv', width: 15 },
            { header: 'Closing Date', key: 'deadline', width: 17 },
            { header: 'Doc Link', key: 'link', width: 50 }
        ];
        tenders.forEach((t, i) => {
            const clean = getCleanTenderData(t);
            worksheet.addRow({
                sno: i + 1,
                tender_id: clean.tenderId,
                title: clean.title,
                ...
                link: clean.finalDocLink || 'N/A'
            });
        });
        worksheet.getRow(1).font = { bold: true };
        const excelBuffer = await workbook.xlsx.writeBuffer();
        const excelFileName = `tenders_${requestId}_${Date.now()}.xlsx`;

        // 4. Format the HTML email body — always use table format
        const tenderListHtml = `
            <div style="overflow-x: auto;">
                <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px; font-size: 13px; font-family: Arial, sans-serif;">
                    <thead>
                        <tr style="background-color: #004d40; color: white; text-align: left;">
                            <th style="padding: 10px; border: 1px solid #e2e8f0; width: 5%;">S.No</th>
                            <th style="padding: 10px; border: 1px solid #e2e8f0; width: 15%;">Tender ID</th>
                            <th style="padding: 10px; border: 1px solid #e2e8f0; width: 40%;">Tender Info</th>
                            <th style="padding: 10px; border: 1px solid #e2e8f0; width: 15%;">Department</th>
                            <th style="padding: 10px; border: 1px solid #e2e8f0; width: 15%;">Closing Date</th>
                            <th style="padding: 10px; border: 1px solid #e2e8f0; width: 10%;">Doc</th>
                        </tr>
                    </thead>
                    <tbody>
                        ${tenders.map((t, index) => {
            const clean = getCleanTenderData(t);
            const linkHtml = clean.finalDocLink ? `<a href="${clean.finalDocLink}" target="_blank" style="color: #10b981; font-weight: bold; text-decoration: none;">View</a>` : '-';
            return `
                            <tr>
                                <td style="padding: 8px; border: 1px solid #e2e8f0; text-align: center;">${index + 1}</td>
                                <td style="padding: 8px; border: 1px solid #e2e8f0;">${clean.tenderId}</td>
                                <td style="padding: 8px; border: 1px solid #e2e8f0; font-weight: bold;">${clean.title}</td>
                                <td style="padding: 8px; border: 1px solid #e2e8f0;">${clean.department}</td>
                                <td style="padding: 8px; border: 1px solid #e2e8f0; color: #b91c1c;">${clean.deadline}</td>
                                <td style="padding: 8px; border: 1px solid #e2e8f0; text-align: center;">${linkHtml}</td>
                            </tr>
                            `;
        }).join('')}
                    </tbody>
            ...
            <div style="background-color: #f0fdf4; border: 1px solid #bbf7d0; padding: 15px; border-radius: 8px; text-align: center; margin-top: 10px;">
                <p style="color: #166534; font-weight: bold; margin: 0;">📎 Full list of ${tenders.length} tender(s) is attached as an Excel file.</p>
            </div>
        `;

        const mailOptions = {
            from: '"BidAlert" <support@bidalert.in>',
            to: userRequest.user_email,
            bcc: 'sobhabidalert@gmail.com',
            subject: 'Found! Best Tenders matching your request',
            headers: defaultMailHeaders,
            text: `Hello ${userRequest.user_name},\n\nWe have searched our database for your requirement: "${userRequest.message}".\n\nWe found ${tenders.length} match(es). Please see the attached Excel file and the email body for the full list.\n\nHappy Bidding!\nThe BidAlert Team`,
            html: `
                <div style="font-family: Arial, sans-serif; background-color: #f8fafc; padding: 40px 20px; color: #333;">
                    <div style="max-width: 700px; margin: 0 auto; background: white; border-radius: 20px; padding: 30px; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
                        <h2 style="color: #004d40; text-transform: uppercase; letter-spacing: -1px; font-size: 28px;">Hello ${userRequest.user_name}!</h2>
                        <p style="font-size: 16px; line-height: 1.6; color: #4a5568;">We have searched our database for your requirement: <strong>"${userRequest.message}"</strong>. Here are the most relevant active tenders found:</p>
                        <hr style="border: 0; border-top: 1px solid #edf2f7; margin: 25px 0;">
                        ${tenderListHtml}
                        <p style="margin-top: 30px; font-size: 14px; color: #718096;">Please review these tenders and click <strong>View</strong> to start your application process.</p>
                        <p style="font-weight: bold; color: #10b981;">Happy Bidding!<br>The BidAlert Team</p>
                    </div>
                    <p style="text-align: center; font-size: 11px; color: #a0aec0; margin-top: 20px;">This email was sent to ${userRequest.user_email} by BidAlert.in</p>
                </div>
            `,
            attachments: [{
        ...
        // Auto-save these tenders for the user so they get expiration alerts
        await saveTendersForUser(userRequest.user_id, tenders);

        res.json({ success: true, message: `Tender list (${tenders.length} tender(s)) sent to user with Excel attachment` });
    } catch (error) {
        console.error('Send tender email error:', error);
        res.status(500).json({ message: 'Server error sending tender email' });
    }
});

// Initialize user_requests table with new columns if needed
const initRequestsTable = async () => {
    try {
        const [columns] = await pool.query('SHOW COLUMNS FROM user_requests LIKE "matches_found"');
        if (columns.length === 0) {
            await pool.query('ALTER TABLE user_requests ADD COLUMN matches_found INT DEFAULT 0 AFTER status');
            console.log('✅ Added matches_found column to user_requests');
        }
    } catch (err) {
        console.warn('⚠️ user_requests table check skipped:', err.message);
    }
};
initRequestsTable();
async function autoProcessAndSendMatches(requestId) {
    try {
        console.log(`🤖 Smart Auto-Match: Processing Request #${requestId}`);

        // 1. Fetch user request + details
        const [requests] = await pool.query(`
            SELECT r.*,
                COALESCE(u.name,  r.user_name)  as user_name,
                COALESCE(u.email, r.user_email) as user_email,
                COALESCE(u.phone, r.user_phone) as user_phone,
                u.id as user_actual_id,
                u.subscription_status as user_sub_status,
                u.plan_expiry_date as user_plan_expiry
            FROM user_requests r
            LEFT JOIN users u ON r.user_id = u.id
            WHERE r.id = ?
        `, [requestId]);

        if (requests.length === 0) return { success: false, message: 'Request not found' };
        const userRequest = requests[0];

        // Safety: if email is somehow still missing, abort gracefully
        if (!userRequest.user_email) {
            console.warn(`⚠️ Request #${requestId} has no email address. Skipping.`);
            await pool.query('UPDATE user_requests SET status = "archived" WHERE id = ?', [requestId]);
            return { success: false, message: 'No email address available for this request' };
        }

        // 1.5 Balance / Subscription Check (Active check for premium auto-worker)
        const isUserFound = !!userRequest.user_actual_id;
        if (isUserFound) {
            const isInvalidSub = !userRequest.user_sub_status || userRequest.user_sub_status === 'expired' || userRequest.user_sub_status === 'cancelled';
            const isExpiredPlan = userRequest.user_plan_expiry && new Date(userRequest.user_plan_expiry) < new Date();

            if (isInvalidSub || isExpiredPlan) {
                console.log(`ℹ️ Request #${requestId} skipped: User sub is ${userRequest.user_sub_status || 'invalid'}. Quota/balance exhausted.`);
                await pool.query('UPDATE user_requests SET status = "archived", admin_notes = ? WHERE id = ?',
                    [`Subscription ${userRequest.user_sub_status || 'invalid'} or Plan expired`, requestId]);
                return { success: false, message: 'Subscription inactive or balance complete' };
            }
        }

        // 2. Extract Details with AI if missing
        let keyword = userRequest.keyword;
        let state = userRequest.state;
        let country = userRequest.country;
        let department = userRequest.department;

        if (!keyword && !state) {
            try {
                const aiResult = await processBidGPTQuery(userRequest.message);
                if (aiResult.type === 'search') {
                    const f = aiResult.filters;
                    keyword = f.q || null;
                    state = f.state || null;
                    country = f.country || null;
                    department = f.category || null;

                    await pool.query(
                        'UPDATE user_requests SET keyword = ?, state = ?, country = ?, department = ? WHERE id = ?',
                        [keyword, state, country, department, requestId]
                    );
                    console.log(`✅ Details extracted for #${requestId}: ${keyword} | ${state}`);
                }
            } catch (aiErr) { console.error('AI Extraction failed:', aiErr.message); }
        }

        // 3. Search for Tenders using extracted details
        const searchParams = {
            q: keyword || userRequest.message,
            state: state || null,
            country: country || null,
            limit: 500
        };

        const matchedData = await getTendersLogic(searchParams);
        const tenders = matchedData.tenders;

        if (tenders.length === 0) {
            console.log(`ℹ️ No matches found for #${requestId}`);
            // Update status and matches_found so it doesn't get processed again
            await pool.query(
                'UPDATE user_requests SET status = "replied", last_sent_at = NOW(), matches_found = 0 WHERE id = ?',
                [requestId]
            );
            return { success: false, message: 'No matching tenders found' };
        }

        // Helper to extract clean titles and valid URLs
        const getCleanTenderData = (t) => {
            let tenderId = t.id || t.tender_id || 'N/A';
            let title = t.title || t.name_of_work || 'N/A';
            let department = t.authority || t.tender_dept || 'Not Specified';
            let state = t.state || t.state_name || 'Not Specified';
            let emd = t.emd || t.tender_emd || 'N/A';
            let ecv = t.estimated_value || t.tender_ecv || 'N/A';
            let deadline = t.deadline || t.closing_date || 'N/A';
        ...
        // 4. Always generate Excel + format email (table format regardless of count)
        const exceljs = require('exceljs');
        const fs = require('fs');
        const path = require('path');
        ...
        worksheet.columns = [
            ...
            { header: 'ECV', key: 'ecv', width: 15 },
            { header: 'Closing Date', key: 'deadline', width: 17 },
            { header: 'Document Link', key: 'link', width: 40 }
        ];
        tenders.forEach((t, i) => {
            const clean = getCleanTenderData(t);
            worksheet.addRow({
        ...
        const fileName = `tenders_${requestId}_${Date.now()}.xlsx`;
        ...
        // Write file for WhatsApp public access
        const exportDir = path.join(__dirname, 'public/exports');
        if (!fs.existsSync(exportDir)) fs.mkdirSync(exportDir, { recursive: true });
        fs.writeFileSync(path.join(exportDir, fileName), buffer);

        const backendUrl = process.env.BACKEND_URL || 'http://localhost:5000';
        const whatsappMediaUrl = `${backendUrl}/exports/${fileName}`;

        // WhatsApp message (always include file link)
        let whatsappMessageText = `Hello ${userRequest.user_name},\n\nWe found *${tenders.length} tenders* matching your request: "${userRequest.message}".\n\nPlease find the full Excel list attached to your email.\n🔗 Direct Download: ${whatsappMediaUrl}`;

        // HTML email body — always table format
        const tenderListHtml = `
            <div style="overflow-x: auto;">
                <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px; font-size: 13px; font-family: Arial, sans-serif;">
                    <thead>
                    ...
                    <tbody>
                        ${tenders.map((t, index) => {
            const clean = getCleanTenderData(t);
            const linkHtml = clean.finalDocLink ? `<a href="${clean.finalDocLink}" target="_blank" style="color: #2563eb; text-decoration: underline;">View</a>` : '-';
            return `
                                <tr>
                                    <td style="padding: 8px; border: 1px solid #e2e8f0; text-align: center;">${index + 1}</td>
                                    <td style="padding: 8px; border: 1px solid #e2e8f0;">${clean.tenderId}</td>
            ...
            <div style="background-color: #f0fdf4; border: 1px solid #bbf7d0; padding: 15px; border-radius: 8px; text-align: center; margin-top: 15px;">
                <p style="color: #166534; font-weight: bold; margin: 0;">📎 Full list of ${tenders.length} tenders attached as an Excel file to this email.</p>
            </div>
        `;

        const mailOptions = {
            from: `"BidAlert" <${process.env.SMTP_USER || 'support@bidalert.in'}>`,
            to: userRequest.user_email,
            bcc: 'sobhabidalert@gmail.com',
            subject: `Found! ${tenders.length} Tenders Matching Your Request — ${userRequest.user_name}`,
            text: `Hi ${userRequest.user_name},\n\nWe found ${tenders.length} tenders matching your request: "${userRequest.message}".\n\nPlease see the attached Excel file for the full list.\n\nHappy Bidding!\nThe BidAlert Team`,
            html: `
                <div style="font-family: Arial, sans-serif; background-color: #f8fafc; padding: 40px 20px; color: #333;">
                    <div style="max-width: 700px; margin: 0 auto; background: white; border-radius: 20px; padding: 30px; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
                        <h2 style="color: #004d40; text-transform: uppercase; letter-spacing: -1px; font-size: 28px;">Hello ${userRequest.user_name}! 🎉</h2>
                        ...
                        <hr style="border: 0; border-top: 1px solid #edf2f7; margin: 25px 0;">
                        ${tenderListHtml}
                        <p style="margin-top: 30px; font-size: 14px; color: #718096;">Please review these tenders and click <strong>View</strong> on any row to open the document.</p>
                        <p style="font-weight: bold; color: #10b981;">Happy Bidding!<br>The BidAlert Team</p>
                    </div>
                    <p style="text-align: center; font-size: 11px; color: #a0aec0; margin-top: 20px;">This email was sent to ${userRequest.user_email} by BidAlert.in</p>
                </div>
        ...
        const log = `[${new Date().toLocaleString()}] 🚀 AUTO-WORKER: Sent matches to ${userRequest.user_email} (${tenders.length} Found)\n`;
        require('fs').appendFileSync(path.join(__dirname, 'email_debug.log'), log);
        console.log(log);

        // Update user_requests with status and the number of matches found
        await pool.query(
            'UPDATE user_requests SET status = "replied", last_sent_at = NOW(), matches_found = ? WHERE id = ?',
            [tenders.length, requestId]
        );

        // Notify Admin that an auto-match was sent
        try {
            await pool.query(
                'INSERT INTO notifications (title, message, type, is_read, user_id, link) VALUES (?, ?, ?, ?, ?, ?)',
                [
                    '🚀 Auto-Sent Tender List',
                    `Request #${requestId} for ${userRequest.user_name} was automatically replied with ${tenders.length} matching tenders via Email.`,
                    'system',
                    0,
                    null,
                    '/admin/messages'
            ...
            console.error('Admin auto-notify failed:', noteErr.message);
        }

        await saveTendersForUser(userRequest.user_actual_id || userRequest.user_id, tenders);

        if (userRequest.user_phone) {
            const cleanName = String(userRequest.user_name || '').substring(0, 15);
            const smsMessage = `Hi ${cleanName}, we found ${tenders.length} matches for your req. Please check your email for details. - BidAlert`;
            sendSMS(userRequest.user_phone, smsMessage).catch(console.error);
        }

        return { success: true, count: tenders.length };
    } catch (err) {
        console.error('autoProcessAndSendMatches error:', err);
        // Safety: update status to stop looping if a crash occurs
        try {
            await pool.query('UPDATE user_requests SET status = "archived", admin_notes = ? WHERE id = ?',
                [`Process Error: ${err.message}`, requestId]);
        } catch (e) { console.error('Failed to clear status on error:', e.message); }
        return { success: false, error: err.message };
    }
}

// Auto-match and send email to user
app.post('/api/admin/auto-send-matches', async (req, res) => {
    try {
        const { requestId } = req.body;
        if (!requestId) return res.status(400).json({ message: 'Request ID is required' });

        const result = await autoProcessAndSendMatches(requestId);

        if (result.success) {
            res.json({ success: true, message: `Found ${result.count} matches and emailed to user.` });
        } else {
            // Correct status code: 404 if request not found, 500 otherwise
            const status = result.message === 'Request not found' ? 404 : 500;
            res.status(status).json({ success: false, message: result.message || result.error || 'Processing failed' });
        }
    } catch (error) {
        console.error('Auto send route error:', error);
        res.status(500).json({ message: 'Server error auto-matching' });
    }
});

app.patch('/api/admin/user-requests/:id', async (req, res) => {
    try {
        const { status } = req.body;
