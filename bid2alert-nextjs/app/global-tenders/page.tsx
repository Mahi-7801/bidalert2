...
// Helper for Highlight — word-boundary regex, only for user-typed keywords
const Highlight = ({ text, query }: { text: string; query: string }) => {
    const keywords = query ? query.split(',').map(k => k.trim()).filter(k => k.length >= 3) : [];
    if (keywords.length === 0) return <>{text}</>;

    const escapedKeywords = keywords.map(k => k.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
    const regexSource = escapedKeywords.map(k => `\\b${k}\\b`).join('|');
    let regex: RegExp;
    try {
...
function GlobalTendersContent() {
    ...
    // Handle auto-focus and scroll for specific filters from navigation
    useEffect(() => {
        const focusField = searchParams.get('focus');
        if (focusField) {
            const el = document.getElementById(`filter-${focusField}`) as HTMLInputElement | null;
                ...
                <div className="container mx-auto px-4 max-w-[1530px]">
                    ...
                    <div className="pt-6 pb-4">
                        <div className="flex items-center gap-2 text-sm text-slate-500 font-medium">
                            ...
                            <span className="text-slate-900 font-bold">Global Tenders</span>
                        </div>
                    </div>
                </div>
                        ...
                        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
                            <div>
                                <h1 className="text-4xl md:text-5xl font-black mb-3 tracking-tight uppercase italic decoration-white">
                                    Global <span className="opacity-95">Tenders</span>
                                </h1>
                                <p className="text-white/95 text-base md:text-lg max-w-2xl font-medium tracking-wide">
                                    Search and discover {totalTenders.toLocaleString()}+ international government contracts worldwide.
                                </p>
                            </div>
                            ...
                            <div className="p-6 space-y-6">
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        className="w-full px-4 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-sm font-bold text-slate-900 focus:outline-none focus:border-bid-green focus:shadow-[0_0_0_4px_rgba(16,185,129,0.15)] transition-all"
                                    />
                                </div>

                                {/* Authority Input */}
                                <div>
                                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Authority / Agency</label>
                                    <input
                                        id="filter-authority"
                                        type="text"
                                        name="authority" value={filters.authority} onChange={handleFilterChange}
                                ...
                                <div>
                                    ...
                                    <p className="text-sm font-medium text-slate-500">
                                        {filters.q ? (
                                            ...
                                            <>
                                                Showing <span className="font-black text-slate-900 text-lg">{(filters.page - 1) * 20 + 1}-{Math.min(filters.page * 20, totalTenders)}</span> of <span className="font-black text-bid-green text-lg">{totalTenders.toLocaleString()}</span> active tenders
                                            </>
                                        )}
                                    </p>
                                </div>
                                                ...
                                                <div className="p-5">
                                                    ...
                                                    <Link href={`/global-tenders/${encodeURIComponent(tender.id)}`}>
                                                        <h2 className="text-lg font-bold text-[#1e293b] leading-tight mb-4 group-hover:text-[#41a367] transition-colors uppercase">
                                                            <Highlight text={tender.title} query={combinedHighlightQuery} />
                                                        </h2>
                                                    </Link>
